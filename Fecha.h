...
// Al revés (5+f) no puede ser un método de la clase, ya que el compilador hace 5.f(operator+(f)
//Por tanto, se hace 'friend' de Fecha:
  friend Fecha operator+(const int &i, const Fecha &f); //const por seguridad y & por velocidad

  friend ostream& operator<<(ostream &s, const Fecha &f);//Necesario para que el compilador sepa que hacer cuando <<fecha
};

// Declaración de la función 'friend' (necesaria si se define fuera)
